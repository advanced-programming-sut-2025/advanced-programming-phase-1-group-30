...
public class ShippingBinScreen {
    ...
    private ArrayList<Item> shippingBinItems = App.getCurrentGame().getCurrentPlayer().getShippingBin().getItems();
    private Item currentItem = null;

    private final Label errorLabel;

    public ShippingBinScreen(SpriteBatch batch, Skin skin) {
        this.skin = skin;
        ...
        moveButton.setPosition(table1.getX() + table1.getWidth() / 2f - moveButton.getWidth() / 2f - 150,
        ...
        numberField = new TextField("", skin);
        numberField.setPosition(table1.getX() + table1.getWidth() / 2f - moveButton.getWidth() / 2f + 50,
            (table1.getY() + table2.getY() + table2.getHeight()) / 2f - moveButton.getHeight() / 2f);
        numberField.setSize(moveButton.getWidth(), moveButton.getHeight());
        numberField.setVisible(false);
        numberField.setTextFieldFilter(new TextField.TextFieldFilter.DigitsOnlyFilter());

        errorLabel = new Label("", skin);
        errorLabel.setColor(Color.RED);
        errorLabel.setPosition(table1.getX() + table1.getWidth() / 2f + 200,
    ...
    private void handleMove() {
        ...
        if (backPackItems.contains(currentItem)) {
            result = GameMenuController.putShippingBin(currentItem, numberField.getText());
        } else if (shippingBinItems.contains(currentItem)) {
            result = GameMenuController.pickShippingBin(currentItem, numberField.getText());
        }
    ...
    private void createItemImage(Item item, float x, float y) {
        ...
        itemImage.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float xOffset, float yOffset) {
                ...
                numberField.setVisible(currentItem != null);
    ...
    private void refresh() {
        ...
        shippingBinItems = App.getCurrentGame().getCurrentPlayer().getShippingBin().getItems();

        renderItemsInGrid(table1.getX() + 50, table1.getY() + table1.getHeight() - 100, backPackItems);
        renderItemsInGrid(table2.getX() + 50, table2.getY() + table2.getHeight() - 100, shippingBinItems);
    }
