...
public enum GameMenuCommands {
    ...
    BUILD("^\\s*build\\s+-a\\s+(?<buildingName>.*\\S)\\s+-l\\s+(?<location>.*\\S)\\s*$"),
    ...
    ANIMALS("^\\s*animals\\s*$"),
    SHEPHERD_ANIMALS("^\\s*shepherd\\s+animals\\s+-n\\s+(?<animalName>.*\\S)\\s+-l\\s+(?<location>.*\\S)\\s*$"),
    ...
    ARTISAN_USE_FIXED("^\\s*artisan\\s+use\\s+Fish_Smoker\\s+Salmon\\s+Coal\\s*$"),
