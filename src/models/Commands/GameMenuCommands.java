...
public enum GameMenuCommands {
    ...
    UPGRADE_TOOL("^\\s*tools\\s+upgrade\\s+(?<name>.+)\\s*$"),
    ...
    TREE_INFO("^\\s*treeinfo\\s+-n\\s+(?<name>.*\\S)\\s*$"),
    FORAGING_CROP_INFO("^\\s*foragingcropinfo\\s+-n\\s+(?<craftName>.*\\S)\\s*$"),
    FORAGING_TREE_INFO("^\\s*foragingtreeinfo\\s+-n\\s+(?<craftName>.*\\S)\\s*$"),
    ...
    CRAFTING_ADD_RECIPE("^\\s*crafting\\s+add\\s+(?<itemName>.*\\S)\\s*$"),
    ...
    ARTISAN_USE("^\\s*artisan\\s+use\\s+-a\\s+(?<artisanName>.*\\S)\\s+-i\\s+(?<itemName>.*\\S)\\s*$"),
